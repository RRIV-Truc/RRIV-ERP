...
(function () {
  ...
  var CFG = window.SPM_GV || {};
  var API = CFG.apiBase || '/api/spm-gv';
  var HUB = CFG.hubUrl || '/';
  var state = {
  ...
  function sessionKey() { return 'spm_gv_session'; }
  ...
  function storeSession(tok) {
    state.session = tok || '';
    try {
      if (tok) localStorage.setItem(sessionKey(), tok);
  ...
  function headers() {
    ...
    else h['X-RRIV-Username'] = username();
    return h;
  }
  ...
  function renderHead() {
    ...
    if ($('btnSyncStaff')) $('btnSyncStaff').hidden = !state.perms.can_manage_staff;
  ...
  function applyBoot(body) {
    state.perms = body.permissions || {};
    state.user = body.user || {};
    if (body.level === 'center' || body.level === 'dept') state.level = body.level;
    else if (state.perms.is_deputy || state.perms.is_head) state.level = 'dept';
    document.querySelectorAll('.gv-tab').forEach(function (b) {
      b.classList.toggle('is-active', b.dataset.level === state.level);
    });
    state.departments = body.departments || [];
    state.staff = body.staff || [];
    state.week = body.current_week;
    state.weeks = body.weeks || [];
    if (!state.week && state.weeks[0]) state.week = state.weeks[0];
    state.board = body.board || null;
    state.unread = body.unread || { count: 0, items: [] };
  ...
  function showSkeleton() {
    var week = $('weekList');
    var body = $('taskBody');
    if (week) week.innerHTML = '<p class="gv-empty gv-skeleton">Đang tải…</p>';
    if (body) body.innerHTML = '<tr><td class="gv-empty gv-skeleton" colspan="11">Đang tải bảng việc…</td></tr>';
  }
  ...
  async function exchangeTicket(ticket) {
    var body = await api('/session', { method: 'POST', body: JSON.stringify({ ticket: ticket }) });
    if (!body.session) throw new Error(body.message || 'Khong doi duoc phien');
  ...
  function startLiveRefresh() {
    ...
    state.pollTimer = setInterval(function () {
      if (document.hidden || !state.week) return;
      loadBoard({ silent: true }).catch(function () {});
    }, 20000);
  }

  function snapshotBoard() {
    try { return JSON.parse(JSON.stringify(state.board || null)); } catch (_) { return state.board; }
  }
  ...
  function pushOptimisticTask(task) {
    if (!state.board) state.board = { tasks: [], groups: [], center_inbox: [], summary: { count: 0, avg_pct: 0, rag: {} } };
    state.board.tasks = (state.board.tasks || []).concat([task]);
    if (task.level === 'center') {
      state.board.center_inbox = (state.board.center_inbox || []).concat([task]);
    } else {
      var groups = state.board.groups || [];
      var g = groups.find(function (x) { return x.parent && x.parent.id === task.parent_id; });
      if (g) g.children = (g.children || []).concat([task]);
      else state.board.orphan_children = (state.board.orphan_children || []).concat([task]);
    }
    if (state.board.summary) state.board.summary.count = (state.board.summary.count || 0) + 1;
    renderStats();
    renderBoard();
  }
  ...
  document.addEventListener('click', function (e) {
    ...
    if (t.id === 'btnSyncStaff') {
      api('/staff/sync', { method: 'POST', body: '{}' })
        .then(function (body) {
          state.staff = body.staff || state.staff;
          renderStaffTable();
          fillSelect($('fLead'), staffForLead(), 'username', 'full_name', true, T('pickPerson'));
          fillSelect($('fNoteUser'), state.staff, 'username', 'full_name');
          fillDatalist();
  ...
  bind('btnSaveAssign', function () {
    ...
    if (!state.editTaskId) {
      pushOptimisticTask({
        ...
        can_report: true,
        can_edit: true,
        can_cascade: payload.level === 'center',
        children_count: 0
      });
    }
    closeM('modalAssign');
  ...
  bind('btnSaveReport', function () {
    var snap = snapshotBoard();
    var task = findTask(state.reportTaskId);
    var pct = Number($('fPct').value || 0);
    var done = $('fDone').value;
    var undone = $('fUndone').value;
    var reason = $('fReason').value;
    if (task) {
      task.progress_pct = pct;
      task.completed_text = done;
      task.incomplete_text = undone;
      task.reason_text = reason;
      renderStats();
      renderBoard();
    }
    closeM('modalReport');
    api('/tasks/' + state.reportTaskId + '/report', {
      ...
      body: JSON.stringify({
        progress_pct: pct,
        completed_text: done,
        incomplete_text: undone,
        reason_text: reason
      })
    }).then(function () { toast(T('okReport')); return loadBoard(); })
      .catch(function (err) { restoreBoard(snap); toast(err.message, true); });
  });
